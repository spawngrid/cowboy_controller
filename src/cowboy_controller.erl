...
upgrade(_ListenerPid, Handler, Opts, Req) ->
	...
	try
		...
		case Mode of
			development ->
				do_render({render_template({views_filename("_exception_error.html", CtrlPriv), [{controller, Handler},{action, Action},
...
render_template({Template, Variables}, #state{ mode = Mode, priv = Priv, app = App, action = Action, handler = Handler } = State) when is_list(Template) ->
   ...
   if 
   	  ((Mode == development) orelse not TemplateLoaded) ->
   	   CompileResult = erlydtl:compile(views_filename(Template, Priv), TemplateModule, [{custom_filters_modules, [cowboy_controller_filters]},
   ...
   case {Mode, CompileResult} of
        ...
        {development, {error, {_File, [{Line, erlydtl_scanner, Reason}]}}} ->
            render_template({views_filename("_template_error.html", CtrlPriv), [{template, Template}, {variables, Variables},
